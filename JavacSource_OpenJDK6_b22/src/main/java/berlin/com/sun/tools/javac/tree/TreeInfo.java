...
public class TreeInfo {
    protected static final Context.Key<TreeInfo> treeInfoKey = new Context.Key<TreeInfo>();
    ...
    public static JCMethodInvocation firstConstructorCall(JCTree tree) {
        ...
        if (stats.isEmpty())
            return null;
        if (stats.head.getTag() != JCTree.EXEC)
            return null;
    ...
    public static int endPos(JCTree tree) {
        if (tree.getTag() == JCTree.BLOCK && ((JCBlock) tree).endpos != Position.NOPOS)
            ...
            JCTry t = (JCTry) tree;
            return endPos((t.finalizer != null) ? t.finalizer : t.catchers.last().body);
        } else
            return tree.pos;
    }
    ...
    /**
     * The end position of given tree, given a table of end positions generated
     * by the parser
     */
    ...
    /**
     * A DiagnosticPosition with the preferred position set to the end position
     * of given tree, if it is a block with defined endpos.
     */
     ...
     * symbol is defined somewhere in the given tree.
     */
    ...
    /**
     * Find the declaration for a symbol, where that symbol is defined somewhere
     * in the given tree.
     */
    ...
    /**
     * Return the statement referenced by a label. If the label refers to a loop
     * or switch, return that switch otherwise return the labelled statement
     * itself
     */
    public static JCTree referencedStatement(JCLabeledStatement tree) {
        JCTree t = tree;
        do
            t = ((JCLabeledStatement) t).body;
     ...
     * its name, otherwise return null.
     */
    ...
    public static final int notExpression = -1, // not an expression
            ...
            assignPrec = 1, assignopPrec = 2, condPrec = 3, orPrec = 4, andPrec = 5, bitorPrec = 6,
            bitxorPrec = 7,
            bitandPrec = 8, eqPrec = 9, ordPrec = 10, shiftPrec = 11, addPrec = 12, mulPrec = 13,
            prefixPrec = 14,
            postfixPrec = 15, precCount = 16;
