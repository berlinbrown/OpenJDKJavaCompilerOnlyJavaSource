...
import static berlin.com.sun.tools.javac.code.Flags.HASINIT;
...
import berlin.com.sun.tools.javac.code.Lint;
...
import berlin.com.sun.tools.javac.util.Bits;
...
public class Flow extends TreeScanner {
    protected static final Context.Key<Flow> flowKey = new Context.Key<Flow>();
    ...
    void errorUncaught() {
        for (PendingExit exit = pendingExits.next(); exit != null; exit = pendingExits.next()) {
            ...
            log.error(exit.tree.pos(), synthetic ? "unreported.exception.default.constructor"
                    : "unreported.exception.need.to.catch.or.throw", exit.thrown);
        }
    }
    ...
    void checkInit(DiagnosticPosition pos, VarSymbol sym) {
        if ((sym.adr >= firstadr || sym.owner.kind != TYP) && trackable(sym) && !inits.isMember(sym.adr)) {
            log.error(pos, "var.might.not.have.been.initialized", sym);
            inits.incl(sym.adr);
        }
    }
    ...
    boolean resolveBreaks(JCTree tree, ListBuffer<PendingExit> oldPendingExits) {
        ...
        for (; exits.nonEmpty(); exits = exits.tail) {
            ...
            if (exit.tree.getTag() == JCTree.BREAK && ((JCBreak) exit.tree).target == tree) {
    ...
    boolean resolveContinues(JCTree tree) {
        ...
        for (; exits.nonEmpty(); exits = exits.tail) {
            PendingExit exit = exits.head;
            if (exit.tree.getTag() == JCTree.CONTINUE && ((JCContinue) exit.tree).target == tree) {
                inits.andSet(exit.inits);
                uninits.andSet(exit.uninits);
                result = true;
            } else {
    ...
    public void visitClassDef(JCClassDecl tree) {
        ...
        try {
            ...
            for (List<JCTree> l = tree.defs; l.nonEmpty(); l = l.tail) {
                if (l.head.getTag() != JCTree.METHODDEF && (TreeInfo.flags(l.head) & STATIC) != 0) {
                    scanDef(l.head);
                    errorUncaught();
                }
            }
            ...
            if (tree.name != names.empty) {
                ...
                for (List<JCTree> l = tree.defs; l.nonEmpty(); l = l.tail) {
                    if (TreeInfo.isInitialConstructor(l.head)) {
                        List<Type> mthrown = ((JCMethodDecl) l.head).sym.type.getThrownTypes();
                        if (firstConstructor) {
                            caught = mthrown;
    ...
    public void visitDoLoop(JCDoWhileLoop tree) {
        ...
        do {
            ...
            scanCond(tree.cond);
            if (log.nerrors != 0 || loopPassTwo || uninitsEntry.diffSet(uninitsWhenTrue).nextBit(firstadr) == -1)
                break;
            inits = initsWhenTrue;
    ...
    public void visitWhileLoop(JCWhileLoop tree) {
        ...
        do {
            ...
            scanStat(tree.body);
            alive |= resolveContinues(tree);
            if (log.nerrors != 0 || loopPassTwo || uninitsEntry.diffSet(uninits).nextBit(firstadr) == -1)
                break;
    ...
    public void visitForLoop(JCForLoop tree) {
        ...
        do {
            ...
            alive |= resolveContinues(tree);
            scan(tree.step);
            if (log.nerrors != 0 || loopPassTwo || uninitsEntry.dup().diffSet(uninits).nextBit(firstadr) == -1)
                break;
            uninits = uninitsEntry.andSet(uninits);
            loopPassTwo = true;
            alive = true;
    ...
    public void visitSwitch(JCSwitch tree) {
        ...
        for (List<JCCase> l = tree.cases; l.nonEmpty(); l = l.tail) {
            ...
            if (!loopPassTwo && alive && lint.isEnabled(Lint.LintCategory.FALLTHROUGH) && c.stats.nonEmpty()
                    && l.tail.nonEmpty())
                log.warning(l.tail.head.pos(), "possible.fall-through.into.case");
        }
        if (!hasDefault) {
    ...
    public void visitTry(JCTry tree) {
        ...
        for (List<JCCatch> l = tree.catchers; l.nonEmpty(); l = l.tail) {
            ...
            if (chk.subset(exc, caughtInTry)) {
                log.error(l.head.pos(), "except.already.caught", exc);
            } else if (!chk.isUnchecked(l.head.pos(), exc) && exc.tsym != syms.throwableType.tsym
                    && exc.tsym != syms.exceptionType.tsym && !chk.intersects(exc, thrownInTry)) {
                log.error(l.head.pos(), "except.never.thrown.in.try", exc);
            }
        ...
        if (tree.finalizer != null) {
            ...
            if (!alive) {
                ...
                if (!loopPassTwo && lint.isEnabled(Lint.LintCategory.FINALLY)) {
                    log.warning(TreeInfo.diagEndPos(tree.finalizer), "finally.cannot.complete");
                }
            } else {
    ...
    public void visitTypeCast(JCTypeCast tree) {
        ...
        if (!tree.type.isErroneous() && lint.isEnabled(Lint.LintCategory.CAST)
