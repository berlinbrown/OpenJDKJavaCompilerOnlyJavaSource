...
import static berlin.javax.tools.StandardLocation.CLASS_PATH;
import static berlin.javax.tools.StandardLocation.PLATFORM_CLASS_PATH;
import static berlin.javax.tools.StandardLocation.SOURCE_PATH;
...
import berlin.com.sun.tools.javac.code.BoundKind;
...
import berlin.com.sun.tools.javac.comp.Annotate;
import berlin.com.sun.tools.javac.file.BaseFileObject;
import berlin.com.sun.tools.javac.util.ArrayUtils;
...
import berlin.javax.lang.model.SourceVersion;
import berlin.javax.tools.JavaFileManager;
import berlin.javax.tools.JavaFileObject;
import berlin.javax.tools.StandardJavaFileManager;
import berlin.javax.tools.JavaFileManager.Location;

/**
 * This class provides operations to read a classfile into an internal
 * representation. The internal representation is anchored in a ClassSymbol
 * which contains in its scope symbol representations for all other definitions
 * in the classfile. Top-level Classes themselves appear as members of the
 * scopes of PackageSymbols.
 */
public class ClassReader extends ClassFile implements Completer {
    /**
     * The context key for the class reader.
     */
    protected static final Context.Key<ClassReader> classReaderKey = new Context.Key<ClassReader>();
    ...
    /**
     * Switch: prefer source files instead of newer when both source and class
     * are available
     **/
    ...
    /**
     * Can be reassigned from outside: the completer to be used for ".java"
     * files. If this remains unassigned ".java" files will not be loaded.
     */
    ...
    /**
     * For every constant pool entry, an index into buf where the defining
     * section of the entry is found.
     */
    int[] poolIdx;

    /**
     * A table to hold the constant pool indices for method parameter names, as
     * given in LocalVariableTable attributes.
     */
    ...
    /**
     * Initialize classes and packages, treating this as the definitive
     * classreader.
     */
    ...
    /**
     * Initialize classes and packages, optionally treating this as the
     * definitive classreader.
     */
    ...
    /**
     * Construct a new class reader, optionally treated as the definitive
     * classreader for this invocation.
     */
    protected ClassReader(Context context, boolean definitive) {
        ...
        completionFailureName = (options.get("failcomplete") != null) ? names.fromString(options.get("failcomplete"))
                : null;
    ...
    public BadClassFile badClassFile(String key, Object... args) {
        return new BadClassFile(currentOwner.enclClass(), currentClassFile, Log.getLocalizedString(key, args));
    }
     ...
     */
    int getInt(int bp) {
        return ((buf[bp] & 0xFF) << 24) + ((buf[bp + 1] & 0xFF) << 16) + ((buf[bp + 2] & 0xFF) << 8)
                + (buf[bp + 3] & 0xFF);
    }
    ...
    Object readPool(int i) {
        ...
        case CONSTANT_NameandType:
            poolObj[i] = new NameAndType(readName(getChar(index + 1)), readType(getChar(index + 3)));
            break;
        case CONSTANT_Integer:
    ...
    /**
     * If name is an array type or class signature, return the corresponding
     * type; otherwise return a ClassSymbol with given name.
     */
    Object readClassOrType(int i) {
        ...
        return (buf[start] == '[' || buf[start + len - 1] == ';') ? (Object) sigToType(buf, start, len)
    ...
    Type sigToType() {
        ...
        case 'T':
            ...
            while (signature[sigp] != ';')
                sigp++;
            sigp++;
            return sigEnterPhase ? Type.noType : findTypeVar(names.fromUtf(signature, start, sigp - 1 - start));
        case '+': {
            sigp++;
            Type t = sigToType();
        ...
        case 'L': {
            ...
            /*
             * System.err.println(" decoded " + new String(signature, oldsigp,
             * sigp-oldsigp) + " => " + t + " outer " + t.outer());
             */
            return t;
        }
        case 'S':
        ...
        case '(':
            ...
            return new MethodType(argtypes, restype, thrown.reverse(), syms.methodClass);
        case '<':
    ...
    Type classSigToType() {
        ...
        while (true) {
            ...
            case ';': { // end
                ClassSymbol t = enterClass(names.fromUtf(signatureBuffer, startSbp, sbp - startSbp));
                        ...
                        if (!completed) {
                            ...
                            if (enclosingType != Type.noType) {
                                ...
                                if (typeParams.length() != typeArgs.length()) {
                                    ...
                                    super.setEnclosingType(types.subst(enclosingType, typeParams, typeArgs));
                                }
                            } else {
    ...
    /**
     * Convert (implicit) signature to list of types until `terminator' is
     * encountered.
     */
    ...
    /**
     * Convert signature to type parameters, where signature is a byte array
     * segment.
     */
                ...
                for (int i = 0; i < numEntries; i++) {
                    ...
                    if (start_pc == 0) {
                        ...
                        if (register >= parameterNameIndices.length) {
                            ...
                            parameterNameIndices = ArrayUtils.copyOf(parameterNameIndices, newSize);
                        }
    ...
    private MethodSymbol findMethod(NameAndType nt, Scope scope, long flags) {
        ...
        nt.type = new MethodType(nt.type.getParameterTypes().tail, nt.type.getReturnType(), nt.type.getThrownTypes(),
                syms.methodClass);
        // Try searching again
    ...
    private boolean isSameBinaryType(MethodType mt1, MethodType mt2) {
        List<Type> types1 = types.erasure(mt1.getParameterTypes()).prepend(types.erasure(mt1.getReturnType()));
    ...
    /**
     * Character.isDigit answers <tt>true</tt> to some non-ascii digits. This
     * one does not. <b>copied from java.lang.Class</b>
     */
    ...
    static class CompoundAnnotationProxy extends Attribute {
        ...
        public String toString() {
            ...
            buf.append("/*proxy*/{");
            boolean first = true;
            for (List<Pair<Name, Attribute>> v = values; v.nonEmpty(); v = v.tail) {
    ...
    class AnnotationDeproxy implements ProxyVisitor {
        private ClassSymbol requestingOwner = currentOwner.kind == MTH ? currentOwner.enclClass()
                : (ClassSymbol) currentOwner;
        ...
        Attribute.Compound deproxyCompound(CompoundAnnotationProxy a) {
            ...
            for (List<Pair<Name, Attribute>> l = a.values; l.nonEmpty(); l = l.tail) {
                MethodSymbol meth = findAccessMethod(a.type, l.head.fst);
                buf.append(new Pair<Symbol.MethodSymbol, Attribute>(meth,
                        deproxy(meth.type.getReturnType(), l.head.snd)));
            }
        ...
        MethodSymbol findAccessMethod(Type container, Name name) {
            ...
            try {
                for (Scope.Entry e = container.tsym.members().lookup(name); e.scope != null; e = e.next()) {
                    Symbol sym = e.sym;
            ...
            try {
                if (failure == null) {
                    log.warning("annotation.method.not.found", container, name);
                } else {
                    log.warning("annotation.method.not.found.reason", container, name, failure.getMessage());
                }
            } finally {
            ...
            MethodType mt = new MethodType(List.<Type> nil(), syms.botType, List.<Type> nil(), syms.methodClass);
        ...
        public void visitEnumAttributeProxy(EnumAttributeProxy proxy) {
            ...
            for (Scope.Entry e = enumTypeSym.members().lookup(proxy.enumerator); e.scope != null; e = e.next()) {
                if (e.sym.kind == VAR) {
            ...
            if (enumerator == null) {
                log.error("unknown.enum.constant", currentClassFile, enumTypeSym, proxy.enumerator);
    ...
    class AnnotationCompleter extends AnnotationDeproxy implements Annotate.Annotator {
        ...
        public void enterAnnotation() {
            ...
            try {
                ...
                sym.attributes_field = ((sym.attributes_field == null) ? newList : newList
                        .prependList(sym.attributes_field));
            } finally {
                currentClassFile = previousClassFile;
            }
        }
    ...
    MethodSymbol readMethod() {
        ...
        if (name == names.init && currentOwner.hasOuterInstance()) {
            ...
            if (currentOwner.name.len != 0)
                type = new MethodType(type.getParameterTypes().tail, type.getReturnType(), type.getThrownTypes(),
                        syms.methodClass);
        }
    ...
    /**
     * Init the parameter names array. Parameter names are currently inferred
     * from the names in the LocalVariableTable attributes of a Code attribute.
     * (Note: this means parameter names are currently not available for methods
     * without a Code attribute.) This method initializes an array in which to
     * store the name indexes of parameter names found in LocalVariableTable
     * attributes. It is slightly supersized to allow for additional slots with
     * a start_pc of 0.
     */
    void initParameterNames(MethodSymbol sym) {
        // make allowance for synthetic parameters.
        final int excessSlots = 4;
        int expectedParameterSlots = Code.width(sym.type.getParameterTypes()) + excessSlots;
        if (parameterNameIndices == null || parameterNameIndices.length < expectedParameterSlots) {
     ...
     * while reading the method attributes (see the Signature attribute). This
     * may be because of generic information or because anonymous synthetic
     * parameters were added. The original type (as read from the method
     * descriptor) is used to help guess the existence of anonymous synthetic
     * parameters. On completion, sym.savedParameter names will either be null
     * (if no parameter names were found in the class file) or will be set to a
     * list of names, one per entry in sym.type.getParameterTypes, with any
     * missing names represented by the empty name.
     */
    void setParameterNames(MethodSymbol sym, Type jvmType) {
        ...
        for (Type t : sym.type.getParameterTypes()) {
            int nameIdx = (index < parameterNameIndices.length ? parameterNameIndices[index] : 0);
    ...
    private void readClassFile(ClassSymbol c) throws IOException {
        ...
        if (majorVersion > maxMajor
                || majorVersion * 1000 + minorVersion < Target.MIN().majorVersion * 1000 + Target.MIN().minorVersion) {
            if (majorVersion == (maxMajor + 1))
                log.warning("big.major.version", currentClassFile, majorVersion, maxMajor);
            else
                throw badClassFile("wrong.version", Integer.toString(majorVersion), Integer.toString(minorVersion),
                        Integer.toString(maxMajor), Integer.toString(maxMinor));
        } else if (checkClassFile && majorVersion == maxMajor && minorVersion > maxMinor) {
    ...
    /**
     * Creates a new toplevel class symbol with given flat name and given class
     * (or source) file.
     * 
     * @param flatName
     *            a fully qualified binary class name
     * @param classFile
     *            the class file or compilation unit defining the class (may be
     *            {@code null})
     * @return a newly created class symbol
     * @throws AssertionError
     *             if the class symbol already exists
     */
    public ClassSymbol enterClass(Name flatName, JavaFileObject classFile) {
        ClassSymbol cs = classes.get(flatName);
        if (cs != null) {
            String msg = Log.format("%s: completer = %s; class file = %s; source file = %s", cs.fullname, cs.completer,
                    cs.classfile, cs.sourcefile);
            throw new AssertionError(msg);
        }
        Name packageName = Convert.packagePart(flatName);
        PackageSymbol owner = packageName.isEmpty() ? syms.unnamedPackage : enterPackage(packageName);
    ...
    /**
     * Create a new member or toplevel class symbol with given flat name and
     * enter in `classes' unless already there.
     */
     ...
     * sure its enclosing class (if any) is loaded.
     */
    public void complete(Symbol sym) throws CompletionFailure {
        if (sym.kind == TYP) {
            ClassSymbol c = (ClassSymbol) sym;
            c.members_field = new Scope.ErrorScope(c); // make sure it's always
            // defined
    ...
    /**
     * Tries to complete lexically enclosing classes if c looks like a nested
     * class. This is similar to completeOwners but handles the situation when a
     * nested class is accessed directly as it is possible with the Tree API or
     * javax.lang.model.*.
     */
    ...
    /**
     * We can only read a single class file at a time; this flag keeps track of
     * when we are currently reading a class file.
     */
    ...
    private void fillIn(ClassSymbol c) {
        ...
        if (classfile != null) {
            JavaFileObject previousClassFile = currentClassFile;
            try {
                assert !filling : "Filling " + classfile.toUri() + " during " + previousClassFile;
                currentClassFile = classfile;
                if (verbose) {
            ...
            throw newCompletionFailure(c, Log.getLocalizedString("class.file.not.found", c.flatname));
        }
    }
    ...
    private static byte[] readInputStream(byte[] buf, InputStream s) throws IOException {
        try {
            ...
            try {
                ...
                /*
                 * Ignore any errors, as this stream may have already thrown a
                 * related exception which is the one that should be reported.
                 */
            }
        }
    }
    ...
    /**
     * Static factory for CompletionFailure objects. In practice, only one can
     * be used at a time, so we share one to reduce the expense of allocating
     * new exception objects.
     */
    ...
    private CompletionFailure cachedCompletionFailure = new CompletionFailure(null, null);
    {
        cachedCompletionFailure.setStackTrace(new StackTraceElement[0]);
    }

    /**
     * Load a toplevel class with given fully qualified name The class is
     * entered into `classes' only if load was successful.
     */
    ...
    protected void includeClassFile(PackageSymbol p, JavaFileObject file) {
        ...
        if (c == null) {
            ...
            if (c.classfile == null) // only update the file if's it's newly
                // created
                c.classfile = file;
            if (isPkgInfo) {
                p.package_info = c;
    ...
    /**
     * Implement policy to choose to derive information from a source file or a
     * class file when both are present. May be overridden by subclasses.
     */
    ...
    private void fillIn(PackageSymbol p) throws IOException {
        ...
        fillIn(p, PLATFORM_CLASS_PATH, fileManager.list(PLATFORM_CLASS_PATH, packageName, EnumSet
                .of(JavaFileObject.Kind.CLASS), false));
    ...
    /**
     * A subclass of JavaFileObject for the sourcefile attribute found in a
     * classfile. The attribute is only the last component of the original
     * filename, so is unlikely to be valid as is, so operations other than
     * those to access the name throw UnsupportedOperationException
     */
    private static class SourceFileObject extends BaseFileObject {
        ...
        public SourceFileObject(Name name, Name flatname) {
            super(null); // no file manager; never referenced for this file
            // object
            this.name = name;
        ...
        /**
         * Check if two file objects are equal. SourceFileObjects are just
         * placeholder objects for the value of a SourceFile attribute, and do
         * not directly represent specific files. Two SourceFileObjects are
         * equal if their names are equal.
         */
        // @Override
