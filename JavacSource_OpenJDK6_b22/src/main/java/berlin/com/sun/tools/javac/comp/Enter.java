...
/**
 * This class enters symbols for all encountered definitions into the symbol
 * table. The pass consists of two phases, organized as follows:
 */
public class Enter extends JCTree.Visitor {
    ...
    /**
     * A hashtable mapping classes and packages to the environments current at
     * the points of their definitions.
     */
    ...
    /**
     * The queue of all classes that might still need to be completed; saved and
     * initialized by main().
     */
     ...
     * scope for local symbols of a class, referred to by the environments
     * info.scope field. This scope will contain - symbols for this and super -
     * symbols for any type parameters In addition, it serves as an anchor for
     * scopes of methods and initializers which are nested in this scope via
     * Scope.dup(). This scope should not be confused with the members scope of
     * a class.
     * 
     * @param tree
     *            The class definition.
     * @param env
     *            The environment current outside of the class definition.
     */
    ...
    /**
     * The scope in which a member definition in environment env is to be
     * entered This is usually the environment's scope, except for class
     * environments, where the local scope is for type variables, and the this
     * and super symbol only, and members go into the class member scope.
     */
    ...
    /**
     * Visitor method: enter all classes in given tree, catching any completion
     * failure exceptions. Return the tree's type.
     * 
     * @param tree
    ...
    /**
     * Visitor method: enter classes of a list of trees, returning a list of
     * types.
     */
        ...
        if (isPkgInfo) {
            ...
            if (env0 == null) {
                ...
                if (!fileManager.isSameFile(tree.sourcefile, tree0.sourcefile)) {
                    ...
                    if (addEnv
                            || (tree0.packageAnnotations.isEmpty() && tree.docComments != null && tree.docComments
                                    .get(tree) != null)) {
                        typeEnvs.put(tree.packge, env);
                    }
                }
    ...
    public void visitClassDef(JCClassDecl tree) {
        ...
        // System.err.println("entering " + c.fullname + " in " +
        // c.owner);//DEBUG
    ...
    /**
     * Class enter visitor method for type parameters. Enter a symbol for type
     * parameter in local scope, after checking that it is unique.
     */
    public void visitTypeParameter(JCTypeParameter tree) {
        TypeVar a = (tree.type != null) ? (TypeVar) tree.type : new TypeVar(tree.name, env.info.scope.owner,
                syms.botType);
        tree.type = a;
     ...
     * 
     * @param trees
     *            The list of trees to be processed.
     */
    ...
    /**
     * Main method: enter one class from a list of toplevel trees and place the
     * rest on uncompleted for later processing.
     * 
     * @param trees
     *            The list of trees to be processed.
     * @param c
