...
package berlin.com.sun.tools.javac.comp;

import static berlin.com.sun.tools.javac.code.Flags.BLOCK;
...
import static berlin.com.sun.tools.javac.code.Flags.PARAMETER;
...
import berlin.com.sun.tools.javac.code.Flags;
...
import berlin.com.sun.tools.javac.jvm.ClassWriter;
...
import berlin.com.sun.tools.javac.tree.TreeScanner;
import berlin.com.sun.tools.javac.tree.TreeTranslator;
...
import berlin.com.sun.tools.javac.tree.JCTree.JCStatement;
...
import berlin.com.sun.tools.javac.util.Convert;
...
import berlin.com.sun.tools.javac.util.JCDiagnostic.DiagnosticPosition;

/**
 * This pass translates away some syntactic sugar: inner classes, class
 * literals, assertions, foreach loops, etc.
 */
public class Lower extends TreeTranslator {
    ...
    protected Lower(Context context) {
        ...
        debugLower = null != options.get("debuglower");
    }
    ...
    /**
     * A hash table mapping virtual accessed symbols in outer subclasses to the
     * actually referred symbol in superclasses.
     */
    ...
    /**
     * A navigator class for assembling a mapping from local class symbols to
     * class definition trees. There is only one case; all other cases simply
    ...
    /**
     * A hash table mapping class symbols to lists of free variables. accessed
     * by them. Only free variables of the method immediately containing a class
     * are associated with that class.
     */
    Map<ClassSymbol, List<VarSymbol>> freevarCache;

    /**
     * A navigator class for collecting the free variables accessed from a local
     * class. There is only one case; all other cases simply traverse down the
     * tree.
     */
    class FreeVarCollector extends TreeScanner {
        ...
        /**
         * Add all free variables of class c to fvs list unless they are already
         * there.
         */
        ...
        /**
         * If tree refers to a variable in owner of local class, add it to free
         * variables list.
         */
        ...
        private void visitSymbol(Symbol _sym) {
            ...
            if (sym.kind == VAR || sym.kind == MTH) {
                while (null != sym && sym.owner != owner)
                    sym = proxies.lookup(proxyName(sym.name)).sym;
                if (null != sym && sym.owner == owner) {
                    VarSymbol v = (VarSymbol) sym;
                    if (null == v.getConstValue()) {
                        addFreeVar(v);
                    }
                } else {
                    if (null != outerThisStack.head && outerThisStack.head != _sym)
        ...
        /**
         * If tree refers to a class instance creation expression add all free
         * variables of the freshly created class.
         */
        public void visitNewClass(JCNewClass tree) {
            ClassSymbol c = (ClassSymbol) tree.constructor.owner;
            addFreeVars(c);
            if (null == tree.encl && c.hasOuterInstance() && null != outerThisStack.head)
                visitSymbol(outerThisStack.head);
            super.visitNewClass(tree);
        }

        /**
         * If tree refers to a qualified this or super expression for anything
         * but the current class, add the outer this stack as a free variable.
         */
        public void visitSelect(JCFieldAccess tree) {
            if ((tree.name == names._this || tree.name == names._super) && tree.selected.type.tsym != clazz
                    && null != outerThisStack.head)
                visitSymbol(outerThisStack.head);
            super.visitSelect(tree);
        }

        /**
         * If tree refers to a superclass constructor call, add all free
        ...
        public void visitApply(JCMethodInvocation tree) {
            if (TreeInfo.name(tree.meth) == names._super) {
                ...
                if (c.hasOuterInstance() && tree.meth.getTag() != JCTree.SELECT && null != outerThisStack.head)
                    visitSymbol(outerThisStack.head);
            }
    ...
    /**
     * Return the variables accessed from within a local class, which are
     * declared in the local class' owner. (in reverse order of first access).
     */
    List<VarSymbol> freevars(ClassSymbol c) {
        if (0 != (c.owner.kind & (VAR | MTH))) {
            List<VarSymbol> fvs = freevarCache.get(c);
            if (null == fvs) {
    ...
    /**
     * This map gives a translation table to be used for enum switches. class
     * EnumMapping provides mapping data and support methods for this
     * translation.
     */
    class EnumMapping {
        EnumMapping(DiagnosticPosition pos, TypeSymbol forEnum) {
            ...
            Name varName = names.fromString(target.syntheticNameChar()
                    ...
                    + writer.xClassName(forEnum.type).toString().replace('/', '.').replace('.',
                            target.syntheticNameChar()));
            ClassSymbol outerCacheClass = outerCacheClass();
            this.mapVar = new VarSymbol(STATIC | SYNTHETIC | FINAL, varName, new ArrayType(syms.intType,
                    syms.arrayClass), outerCacheClass);
        ...
        void translate() {
            ...
            MethodSymbol valuesMethod = lookupMethod(pos, names.values, forEnum.type, List.<Type> nil());
            JCExpression size = make // Color.values().length
                    .Select(make.App(make.QualIdent(valuesMethod)), syms.lengthVar);
            JCExpression mapVarInit = make.NewArray(make.Type(syms.intType), List.of(size), null).setType(
                    new ArrayType(syms.intType, syms.arrayClass));

            // try { $SwitchMap$Color[red.ordinal()] = 1; } catch
            // (java.lang.NoSuchFieldError ex) {}
            ListBuffer<JCStatement> stmts = new ListBuffer<JCStatement>();
            Symbol ordinalMethod = lookupMethod(pos, names.ordinal, forEnum.type, List.<Type> nil());
            List<JCCatch> catcher = List.<JCCatch> nil().prepend(
                    make.Catch(make.VarDef(
                            new VarSymbol(PARAMETER, names.ex, syms.noSuchFieldErrorType, syms.noSymbol), null), make
            ...
            for (Map.Entry<VarSymbol, Integer> e : values.entrySet()) {
                ...
                JCExpression assign = make.Assign(
                        make.Indexed(mapVar, make.App(make.Select(make.QualIdent(enumerator), ordinalMethod))),
                        make.Literal(mappedValue)).setType(syms.intType);
                JCStatement exec = make.Exec(assign);
                JCStatement _try = make.Try(make.Block(0, List.of(exec)), catcher, null);
                stmts.append(_try);
            }

            owner.defs = owner.defs.prepend(make.Block(STATIC, stmts.toList()))
    ...
    /**
     * Make an attributed tree representing a literal. This will be an Ident
     * node in the case of boolean literals, a Literal node in all other cases.
     * 
     * @param type
     *            The literal's type.
     * @param value
    ...
    JCNewClass makeNewClass(Type ctype, List<JCExpression> args) {
        ...
        tree.constructor = rs.resolveConstructor(make_pos, attrEnv, ctype, TreeInfo.types(args), null, false, false);
        tree.type = ctype;
        return tree;
    }
    ...
    JCUnary makeUnary(int optag, JCExpression arg) {
        ...
        tree.operator = rs.resolveUnaryOperator(make_pos, optag, attrEnv, arg.type);
    ...
    JCBinary makeBinary(int optag, JCExpression lhs, JCExpression rhs) {
        ...
        tree.operator = rs.resolveBinaryOperator(make_pos, optag, attrEnv, lhs.type, rhs.type);
        tree.type = tree.operator.type.getReturnType();
        return tree;
    }
    ...
    JCAssignOp makeAssignop(int optag, JCTree lhs, JCTree rhs) {
        ...
        tree.operator = rs.resolveBinaryOperator(make_pos, tree.getTag() - JCTree.ASGOffset, attrEnv, lhs.type,
                rhs.type);
        tree.type = lhs.type;
        return tree;
    }
    ...
    JCExpression makeString(JCExpression tree) {
        if (tree.type.tag >= CLASS) {
            ...
            Symbol valueOfSym = lookupMethod(tree.pos(), names.valueOf, syms.stringType, List.of(tree.type));
            return make.App(make.QualIdent(valueOfSym), List.of(tree));
        }
    }

    /**
     * Create an empty anonymous class definition and enter and complete its
     * symbol. Return the class definition's symbol. and create
     * 
     * @param flags
     *            The class symbol's flags
     * @param owner
     *            The class symbol's owner
     */
    ClassSymbol makeEmptyClass(long flags, ClassSymbol owner) {
        ...
        JCClassDecl cdef = make.ClassDef(make.Modifiers(flags), names.empty, List.<JCTypeParameter> nil(), null, List
                .<JCExpression> nil(), List.<JCTree> nil());
        cdef.sym = c;
        cdef.type = c.type;
        ...
        if (sym.name != names.error && sym.name != names.empty) {
            for (Scope.Entry e = s.lookup(sym.name); e.scope == s; e = e.next()) {
                if (sym != e.sym && sym.kind == e.sym.kind) {
                    ...
                    if (0 != (sym.kind & (MTH | VAR)) && !types.erasure(sym.type).equals(types.erasure(e.sym.type)))
                        continue;
    ...
    private Symbol lookupSynthetic(Name name, Scope s) {
        ...
        return (null == sym || 0 == (sym.flags() & SYNTHETIC)) ? null : sym;
    }
     ...
     * increment/decrement. Access codes for assignment operations are
     * determined by method accessCode below.
     * 
     * @see Gen.visitSelect(Select tree)
     */
    private static final int DEREFcode = 0, ASSIGNcode = 2, PREINCcode = 4, PREDECcode = 6, POSTINCcode = 8,
            POSTDECcode = 10, FIRSTASGOPcode = 12;
    ...
    /**
     * A mapping from symbols to an array of access symbols, indexed by access
     * code.
     */
    ...
    private static int accessCode(JCTree tree, JCTree enclOp) {
        if (null == enclOp)
            return DEREFcode;
        else if (enclOp.getTag() == JCTree.ASSIGN && tree == TreeInfo.skipParens(((JCAssign) enclOp).lhs))
            return ASSIGNcode;
    ...
    private OperatorSymbol binaryAccessOperator(int acode) {
        for (Scope.Entry e = syms.predefClass.members().elems; null != e; e = e.sibling) {
    ...
    /**
     * Return tree tag for assignment operation corresponding to given binary
     * operator.
     */
    ...
    /**
     * Return access symbol for a private or protected symbol from an inner
     * class.
     */
    MethodSymbol accessSymbol(Symbol sym, JCTree tree, JCTree enclOp, boolean protAccess, boolean refSuper) {
        ...
        if (null == anum) {
            ...
            // System.out.println("accessing " + vsym + " in " +
            // vsym.location());
        }
        ...
        if (null == accessor) {
            accessor = new MethodSymbol(STATIC | SYNTHETIC, accessName(anum.intValue(), acode), new MethodType(
                    argtypes, restype, thrown, syms.methodClass), accOwner);
    ...
    /**
     * The qualifier to be used for accessing a symbol in an outer class. This
     * is either C.sym or C.this.sym, depending on whether or not sym is static.
     */
    JCExpression accessBase(DiagnosticPosition pos, Symbol sym) {
        return ((sym.flags() & STATIC) != 0) ? access(make.at(pos.getStartPosition()).QualIdent(sym.owner))
    ...
    boolean needsPrivateAccess(Symbol sym) {
        if (0 == (sym.flags() & PRIVATE) || sym.owner == currentClass) {
            return false;
        } else if (sym.name == names.init && 0 != (sym.owner.owner.kind & (VAR | MTH))) {
    ...
    boolean needsProtectedAccess(Symbol sym, JCTree tree) {
        if (0 == (sym.flags() & PROTECTED) || sym.owner.owner == currentClass.owner || // fast
                // special
                // case
        ...
        if (!currentClass.isSubClass(sym.owner, types))
            return true;
        if (0 != (sym.flags() & STATIC) || tree.getTag() != JCTree.SELECT
     ...
     *            The access symbol
     * @param protAccess
     *            Is access to a protected symbol in another package?
     */
    ClassSymbol accessClass(Symbol sym, boolean protAccess, JCTree tree) {
        if (protAccess) {
            Symbol qualifier = null;
            ClassSymbol c = currentClass;
            if (tree.getTag() == JCTree.SELECT && 0 == (sym.flags() & STATIC)) {
    ...
    /**
     * Ensure that identifier is accessible, return tree accessing the
     * identifier.
     * 
     * @param sym
     *            The accessed symbol.
     * @param tree
     *            The tree referring to the symbol.
     * @param enclOp
     *            The closest enclosing operation node of tree, null if tree is
     *            not a subtree of an operation.
     * @param refSuper
     *            Is access via a (qualified) C.super?
     */
    JCExpression access(Symbol sym, JCExpression tree, JCExpression enclOp, boolean refSuper) {
        ...
        while (sym.kind == VAR && sym.owner.kind == MTH && sym.owner.enclClass() != currentClass) {
            ...
            sym = proxies.lookup(proxyName(sym.name)).sym;
            assert null != sym && 0 != (sym.flags_field & FINAL);
        ...
        case TYP:
            if (sym.owner.kind != PCK) {
                ...
                while (null != base && null != TreeInfo.symbol(base) && TreeInfo.symbol(base).kind != PCK) {
                    base = (base.getTag() == JCTree.SELECT) ? ((JCFieldAccess) base).selected : null;
                }
        ...
        case VAR:
            if (sym.owner.kind == TYP) {
                ...
                boolean baseReq = null == base && sym.owner != syms.predefClass && !sym.isMemberOf(currentClass, types);
    ...
    /**
     * Return access constructor for a private constructor, or the constructor
     * itself, if no access constructor is needed.
     * 
     * @param pos
     *            The position to report diagnostics, if any.
     * @param constr
    ...
    Symbol accessConstructor(DiagnosticPosition pos, Symbol constr) {
        if (needsPrivateAccess(constr)) {
            ...
            if (aconstr == null) {
                ...
                aconstr = new MethodSymbol(SYNTHETIC, names.init, new MethodType(argtypes.append(accessConstructorTag()
                        .erasure(types)), constr.type.getReturnType(), constr.type.getThrownTypes(), syms.methodClass),
                        accOwner);
    ...
    ClassSymbol accessConstructorTag() {
        ...
        Name flatname = names.fromString("" + topClass.getQualifiedName() + target.syntheticNameChar() + "1");
    ...
    JCTree accessConstructorDef(int pos, Symbol constr, MethodSymbol accessor) {
        ...
        md.body = make.Block(0, List.<JCStatement> of(make.Call(make.App(callee, make.Idents(md.params.reverse().tail
                .reverse())))));
        return md;
    }
    ...
    /**
     * A stack containing the this$n field of the currently translated classes
     * (if needed) in innermost first order.
     */
     ...
     * 
     * @param pos
     *            The source code position of the definition.
     * @param freevars
     *            The free variables.
     * @param owner
     *            The class in which the definitions go.
     */
    List<JCVariableDecl> freevarDefs(int pos, List<VarSymbol> freevars, Symbol owner) {
        ...
        for (List<VarSymbol> l = freevars; l.nonEmpty(); l = l.tail) {
            VarSymbol v = l.head;
            VarSymbol proxy = new VarSymbol(flags, proxyName(v.name), v.erasure(types), owner);
            proxies.enter(proxy);
     ...
     * 
     * @param type
     *            The class referenced by the this$n field
     */
     ...
     *            The source code position of the definition.
     * @param owner
     *            The class in which the definition goes.
     */
    ...
    /**
     * Return a list of trees that load the free variables in given list, in
     * reverse order.
     * 
     * @param pos
     *            The source code position to be used for the trees.
     * @param freevars
    ...
    /**
     * Construct a tree that represents the outer instance <C.this>. Never pick
     * the current `this'.
     * 
     * @param pos
     *            The source code position to be used for the tree.
     * @param c
     *            The qualifier class.
     */
    ...
    /**
     * Construct a tree that represents the closest outer instance <C.this> such
     * that the given symbol is a member of C.
     * 
     * @param pos
     *            The source code position to be used for the tree.
     * @param sym
     *            The accessed symbol.
     * @param preciseMatch
     *            should we accept a type that is a subtype of sym's owner, even
     *            if it doesn't contain sym due to hiding, overriding, or
     *            non-inheritance due to protection?
     */
    ...
    /**
     * Return tree simulating the assignment <this.name = name>, where name is
    ...
    JCStatement initField(int pos, Name name) {
        ...
        make.at(pos);
        return make.Exec(make.Assign(make.Select(make.This(lhs.owner.erasure(types)), lhs), make.Ident(rhs)).setType(
                lhs.erasure(types)));
    }
     ...
     * statics such as class$ and the $assertionsDisabled flag. We create an
     * anonymous nested class (unless one already exists) and return its symbol.
     * However, for backward compatibility in 1.4 and earlier we use the
     * top-level class itself.
     */
    private ClassSymbol outerCacheClass() {
        ClassSymbol clazz = outermostClassDef.sym;
        if ((clazz.flags() & INTERFACE) == 0 && !target.useInnerCacheClass())
            return clazz;
        Scope s = clazz.members();
        for (Scope.Entry e = s.elems; e != null; e = e.sibling)
            if (e.sym.kind == TYP && e.sym.name == names.empty && (e.sym.flags() & INTERFACE) == 0)
                return (ClassSymbol) e.sym;
        return makeEmptyClass(STATIC | SYNTHETIC, clazz);
    }

    /**
     * Return symbol for "class$" method. If there is no method definition for
     * class$, construct one as follows:
     * 
     * class class$(String x0) { try { return Class.forName(x0); } catch
     * (ClassNotFoundException x1) { throw new
     * NoClassDefFoundError(x1.getMessage()); } }
     */
    private MethodSymbol classDollarSym(DiagnosticPosition pos) {
        ClassSymbol outerCacheClass = outerCacheClass();
        MethodSymbol classDollarSym = (MethodSymbol) lookupSynthetic(classDollar, outerCacheClass.members());
        if (classDollarSym == null) {
            classDollarSym = new MethodSymbol(STATIC | SYNTHETIC, classDollar, new MethodType(List.of(syms.stringType),
                    types.erasure(syms.classType), List.<Type> nil(), syms.methodClass), outerCacheClass);
    ...
    JCBlock classDollarSymBody(DiagnosticPosition pos, JCMethodDecl md) {
        ...
        if (target.classLiteralsNoInit()) {
            ...
            VarSymbol clsym = new VarSymbol(STATIC | SYNTHETIC, names.fromString("cl" + target.syntheticNameChar()),
            ...
            JCNewArray newcache = make.NewArray(make.Type(outerCacheClass.type), List.<JCExpression> of(make.Literal(
                    INT, 0).setType(syms.intType)), null);
            ...
            Symbol forNameSym = lookupMethod(make_pos, names.forName, types.erasure(syms.classType), List.of(
                    syms.stringType, syms.booleanType, syms.classLoaderType));
            // clvalue := "(cl$ == null) ?
            // $newcache.getClass().getComponentType().getClassLoader() : cl$"
            JCExpression clvalue = make.Conditional(
                    makeBinary(JCTree.EQ, make.Ident(clsym), makeNull()),
                    make.Assign(
                            make.Ident(clsym),
                            makeCall(makeCall(makeCall(newcache, names.getClass, List.<JCExpression> nil()),
                                    names.getComponentType, List.<JCExpression> nil()), names.getClassLoader, List
                                    .<JCExpression> nil())).setType(syms.classLoaderType), make.Ident(clsym)).setType(
            ...
            returnResult = make.Block(0, List.<JCStatement> of(make.Call(make. // return
                    App(make.Ident(forNameSym), args))));
        } else {
            // forNameSym := java.lang.Class.forName(String s)
            Symbol forNameSym = lookupMethod(make_pos, names.forName, types.erasure(syms.classType), List
                    .of(syms.stringType));
            // returnResult := "{ return Class.forName(param1); }"
            returnResult = make.Block(0, List.of(make.Call(make. // return
                    App(make.QualIdent(forNameSym), List.<JCExpression> of(make.Ident(md.params.head.sym))))));
        }
        ...
        if (target.hasInitCause()) {
            ...
            JCTree throwExpr = makeCall(makeNewClass(syms.noClassDefFoundErrorType, List.<JCExpression> nil()),
                    names.initCause, List.<JCExpression> of(make.Ident(catchParam)));
            rethrow = make.Throw(throwExpr);
        } else {
            // getMessageSym := ClassNotFoundException.getMessage()
            Symbol getMessageSym = lookupMethod(make_pos, names.getMessage, syms.classNotFoundExceptionType, List
                    .<Type> nil());
            // rethrow = "throw new NoClassDefFoundError(e.getMessage());"
            rethrow = make.Throw(makeNewClass(syms.noClassDefFoundErrorType, List.<JCExpression> of(make.App(make
                    .Select(make.Ident(catchParam), getMessageSym), List.<JCExpression> nil()))));
        }
    ...
    /**
     * The variable symbol that caches T.class values. If none exists yet,
     * create a definition.
     * 
     * @param sig
     *            The signature of type T.
    ...
    private VarSymbol cacheSym(DiagnosticPosition pos, String sig) {
        ...
        if (cacheSym == null) {
            cacheSym = new VarSymbol(STATIC | SYNTHETIC, cname, types.erasure(syms.classType), outerCacheClass);
    ...
    private JCExpression classOfType(Type type, DiagnosticPosition pos) {
        ...
        case VOID:
            ...
            Symbol typeSym = rs.access(rs.findIdentInType(attrEnv, c.type, names.TYPE, VAR), pos, c.type, names.TYPE,
                    true);
            if (typeSym.kind == VAR)
                ((VarSymbol) typeSym).getConstValue(); // ensure initializer is
            // evaluated
        ...
        case ARRAY:
            ...
            return make_at(pos)
                    .Conditional(
                            ...
                            make.Assign(
                                    make.Ident(cs),
                                    make.App(make.Ident(classDollarSym(pos)), List.<JCExpression> of(make.Literal(
                                            CLASS, sig).setType(syms.stringType)))).setType(
                                    types.erasure(syms.classType)), make.Ident(cs)).setType(
    ...
    private JCExpression assertFlagTest(DiagnosticPosition pos) {
        ...
        if (assertDisabledSym == null) {
            assertDisabledSym = new VarSymbol(STATIC | FINAL | SYNTHETIC, dollarAssertionsDisabled, syms.booleanType,
                    container);
            enterSynthetic(pos, assertDisabledSym, container.members());
            Symbol desiredAssertionStatusSym = lookupMethod(pos, names.desiredAssertionStatus, types
                    .erasure(syms.classType), List.<Type> nil());
            JCClassDecl containerDef = classDef(container);
            make_at(containerDef.pos());
            JCExpression notStatus = makeUnary(JCTree.NOT, make.App(make.Select(classOfType(types
                    .erasure(outermostClass.type), containerDef.pos()), desiredAssertionStatusSym)));
    ...
    /**
     * Construct an expression using the builder, with the given rval expression
     * as an argument to the builder. However, the rval expression must be
     * computed only once, even if used multiple times in the result of the
     * builder. We do that by constructing a "let" expression that saves the
     * rvalue into a temporary variable and then uses the temporary variable in
     * place of the expression built by the builder. The complete resulting
     ...
     * </pre>
     * 
     * where <code><b>TEMP</b></code> is a newly declared variable in the let
     * expression.
     */
    JCTree abstractRval(JCTree rval, Type type, TreeBuilder builder) {
        ...
        VarSymbol var = new VarSymbol(FINAL | SYNTHETIC, Name.fromString(names, target.syntheticNameChar() + ""
                + rval.hashCode()), type, currentMethodSym);
    ...
    /**
     * Visitor method: Translate a single node. Attach the source position from
     * the old tree to its replacement tree.
     */
    ...
    public void visitTopLevel(JCCompilationUnit tree) {
        if (tree.packageAnnotations.nonEmpty()) {
            ...
            if (target.isPackageInfoSynthetic())
                // package-info is marked SYNTHETIC in JDK 1.6 and later
                // releases
                flags = flags | Flags.SYNTHETIC;
            JCClassDecl packageAnnotationsClass = make.ClassDef(make.Modifiers(flags, tree.packageAnnotations), name,
    ...
    private void visitEnumDef(JCClassDecl tree) {
        ...
        // process each enumeration constant, adding implicit constructor
        // parameters
        int nextOrdinal = 0;
        ...
        JCNewArray newArray = make.NewArray(make.Type(types.erasure(tree.type)), List.<JCExpression> nil(), values
                .toList());
        ...
        if (useClone()) {
            ...
            JCTypeCast valuesResult = make.TypeCast(valuesSym.type.getReturnType(), make.App(make.Select(make
                    .Ident(valuesVar), syms.arrayCloneMethod)));
            ...
            JCNewArray resultArray = make.NewArray(make.Type(types.erasure(tree.type)), List.of(make.Select(make
                    .Ident(valuesVar), syms.lengthVar)), null);
            resultArray.type = arrayType;
            JCVariableDecl decl = make.VarDef(resultVar, resultArray);
            if (systemArraycopyMethod == null) {
                systemArraycopyMethod = new MethodSymbol(PUBLIC | STATIC, names.fromString("arraycopy"),
                        new MethodType(List.<Type> of(syms.objectType, syms.intType, syms.objectType, syms.intType,
                                syms.intType), syms.voidType, List.<Type> nil(), syms.methodClass),
                        syms.systemType.tsym);
            }
            JCStatement copy = make.Exec(make.App(make.Select(make.Ident(syms.systemType.tsym), systemArraycopyMethod),
                    List.of(make.Ident(valuesVar), make.Literal(0), make.Ident(resultVar), make.Literal(0), make
                            .Select(make.Ident(valuesVar), syms.lengthVar))));
         ...
         * 
         * public static E valueOf(String name) { return
         * (E)Enum.valueOf(E.class, name); }
         * 
         * where E is tree.sym
         */
        ...
        JCStatement enum_ValueOf = make.Return(make.TypeCast(tree.sym.type, makeCall(make.Ident(syms.enumSym),
                names.valueOf, List.of(e_class, nameVal))));
    ...
    private void visitEnumConstantDef(JCVariableDecl var, int ordinal) {
        ...
        varDef.args = varDef.args.prepend(makeLit(syms.intType, ordinal)).prepend(
                makeLit(syms.stringType, var.name.toString()));
    }

    public void visitMethodDef(JCMethodDecl tree) {
        if (tree.name == names.init && (currentClass.flags_field & ENUM) != 0) {
            // Add "String $enum$name, int $enum$ordinal" to the beginning of
            // the
            // argument list for each constructor of an enum.
            JCVariableDecl nameParam = make_at(tree.pos()).Param(
                    names.fromString(target.syntheticNameChar() + "enum" + target.syntheticNameChar() + "name"),
                    syms.stringType, tree.sym);
            nameParam.mods.flags |= SYNTHETIC;
            nameParam.sym.flags_field |= SYNTHETIC;

            JCVariableDecl ordParam = make.Param(names.fromString(target.syntheticNameChar() + "enum"
                    + target.syntheticNameChar() + "ordinal"), syms.intType, tree.sym);
            ...
            m.erasure_field = new MethodType(olderasure.getParameterTypes().prepend(syms.intType).prepend(
                    syms.stringType), olderasure.getReturnType(), olderasure.getThrownTypes(), syms.methodClass);
    ...
    private void visitMethodDefInternal(JCMethodDecl tree) {
        if (tree.name == names.init && (currentClass.isInner() || (currentClass.owner.kind & (VAR | MTH)) != 0)) {
            ...
            if (fvs.nonEmpty()) {
                ...
                Type olderasure = m.erasure(types);
                m.erasure_field = new MethodType(olderasure.getParameterTypes().appendList(addedargtypes), olderasure
                        .getReturnType(), olderasure.getThrownTypes(), syms.methodClass);
            }
    ...
    public void visitAssert(JCAssert tree) {
        ...
        if (!tree.cond.type.isTrue()) {
            ...
            List<JCExpression> exnArgs = (tree.detail == null) ? List.<JCExpression> nil() : List
                    .of(translate(tree.detail));
    ...
    public void visitApply(JCMethodInvocation tree) {
        ...
        if (meth.name == names.init) {
            ...
            if ((c.flags_field & ENUM) != 0 || c.getQualifiedName() == names.java_lang_Enum) {
                ...
                tree.args = tree.args.prepend(make_at(tree.pos()).Ident(params.tail.head.sym)) // ordinal
            ...
            // We are seeing a normal method invocation; translate this as
            // usual.
    ...
    JCExpression boxPrimitive(JCExpression tree, Type box) {
        ...
        if (target.boxWithConstructors()) {
            Symbol ctor = lookupConstructor(tree.pos(), box, List.<Type> nil().prepend(tree.type));
            return make.Create(ctor, List.of(tree));
        } else {
            Symbol valueOfSym = lookupMethod(tree.pos(), names.valueOf, box, List.<Type> nil().prepend(tree.type));
            return make.App(make.QualIdent(valueOfSym), List.of(tree));
        }
    }
    ...
    JCExpression unbox(JCExpression tree, Type primitive) {
        ...
        make_at(tree.pos());
        Symbol valueSym = lookupMethod(tree.pos(), unboxedType.tsym.name.append(names.Value), // x.intValue()
                tree.type, List.<Type> nil());
        return make.App(make.Select(tree, valueSym));
    }

    /**
     * Visitor method for parenthesized expressions. If the subexpression has
     * changed, omit the parens.
     */
    ...
    public void visitAssignop(final JCAssignOp tree) {
        if (!tree.lhs.type.isPrimitive() && tree.operator.type.getReturnType().isPrimitive()) {
            // boxing required; need to rewrite as x = (unbox typeof x)(x op y);
            // or if x == (typeof x)z then z = (unbox typeof x)((typeof x)z op
            // y)
            // (but without recomputing x)
            JCTree arg = (tree.lhs.getTag() == JCTree.TYPECAST) ? ((JCTypeCast) tree.lhs).expr : tree.lhs;
            JCTree newTree = abstractLval(arg, new TreeBuilder() {
                public JCTree build(final JCTree lhs) {
                    ...
                    Symbol newOperator = rs
                            .resolveBinaryOperator(tree.pos(), newTag, attrEnv, tree.type, tree.rhs.type);
                    ...
                    JCTypeCast newRhs = make.TypeCast(types.unboxedType(tree.type), opResult);
        ...
        if (tree.lhs.getTag() == JCTree.APPLY) {
            ...
            JCExpression rhs = (((OperatorSymbol) tree.operator).opcode == string_add) ? makeString(tree.rhs)
                    : tree.rhs;
            ...
            public JCTree build(final JCTree tmp1) {
                return abstractRval(tmp1, tree.arg.type, new TreeBuilder() {
                    public JCTree build(final JCTree tmp2) {
                        int opcode = (tree.getTag() == JCTree.POSTINC) ? JCTree.PLUS_ASG : JCTree.MINUS_ASG;
    ...
    public void visitUnary(JCUnary tree) {
        boolean isUpdateOperator = JCTree.PREINC <= tree.getTag() && tree.getTag() <= JCTree.POSTDEC;
        if (isUpdateOperator && !tree.arg.type.isPrimitive()) {
            ...
            case JCTree.PREDEC: // -- e
                // translate to e -= 1
            {
                int opcode = (tree.getTag() == JCTree.PREINC) ? JCTree.PLUS_ASG : JCTree.MINUS_ASG;
    ...
    private void visitArrayForeachLoop(JCEnhancedForLoop tree) {
        ...
        VarSymbol arraycache = new VarSymbol(0, names.fromString("arr" + target.syntheticNameChar()), tree.expr.type,
                currentMethodSym);
        JCStatement arraycachedef = make.VarDef(arraycache, tree.expr);
        VarSymbol lencache = new VarSymbol(0, names.fromString("len" + target.syntheticNameChar()), syms.intType,
                currentMethodSym);
        JCStatement lencachedef = make.VarDef(lencache, make.Select(make.Ident(arraycache), syms.lengthVar));
        VarSymbol index = new VarSymbol(0, names.fromString("i" + target.syntheticNameChar()), syms.intType,
                currentMethodSym);
        ...
        JCStatement loopvarinit = make.VarDef(tree.var.sym, make.Indexed(make.Ident(arraycache), make.Ident(index))
                .setType(elemtype));
        JCBlock body = make.Block(0, List.of(loopvarinit, tree.body));
    ...
    private void visitIterableForeachLoop(JCEnhancedForLoop tree) {
        ...
        Symbol iterator = lookupMethod(tree.expr.pos(), names.iterator, types.erasure(syms.iterableType), List
                .<Type> nil());
        VarSymbol itvar = new VarSymbol(0, names.fromString("i" + target.syntheticNameChar()), types
                .erasure(iterator.type.getReturnType()), currentMethodSym);
    ...
    public void visitVarDef(JCVariableDecl tree) {
        ...
        if (currentMethodSym == null) {
            ...
            currentMethodSym = new MethodSymbol((tree.mods.flags & STATIC) | BLOCK, names.empty, null, currentClass);
        }
        if (tree.init != null)
    ...
    public void visitReturn(JCReturn tree) {
        if (tree.expr != null)
            tree.expr = translate(tree.expr, types.erasure(currentMethodDef.restype.type));
        result = tree;
    }
    ...
    public void visitSelect(JCFieldAccess tree) {
        ...
        boolean qualifiedSuperAccess = tree.selected.getTag() == JCTree.SELECT
     ...
     * class and translated versions of all inner classes.
     * 
     * @param env
     *            The attribution environment current at the class definition.
     *            We need this for resolving some additional symbols.
     * @param cdef
     *            The tree representing the class definition.
     */
    ...
    private VarSymbol addEnumOrdinalField(JCClassDecl cdef) {
        VarSymbol ordinal = new VarSymbol(PRIVATE | FINAL | SYNTHETIC, names.fromString("$ordinal"), syms.intType,
                cdef.sym);
    ...
    private VarSymbol addEnumNameField(JCClassDecl cdef) {
        VarSymbol name = new VarSymbol(PRIVATE | FINAL | SYNTHETIC, names.fromString("$name"), syms.stringType,
                cdef.sym);
    ...
    private MethodSymbol addEnumFieldOrdinalMethod(JCClassDecl cdef, VarSymbol ordinalSymbol) {
        ...
        Symbol ordinalSym = lookupMethod(cdef.pos(), names.ordinal, cdef.type, List.<Type> nil());

        assert (ordinalSym != null);
        assert (ordinalSym instanceof MethodSymbol);

        JCStatement ret = make.Return(make.Ident(ordinalSymbol));
        cdef.defs = cdef.defs.append(make.MethodDef((MethodSymbol) ordinalSym, make.Block(0L, List.of(ret))));
    ...
    private MethodSymbol addEnumCompareTo(JCClassDecl cdef, VarSymbol ordinalSymbol) {
        ...
        JCMethodDecl compareToMethod = make.MethodDef((MethodSymbol) compareToSym, make.Block(0L, blockStatements
                .toList()));
