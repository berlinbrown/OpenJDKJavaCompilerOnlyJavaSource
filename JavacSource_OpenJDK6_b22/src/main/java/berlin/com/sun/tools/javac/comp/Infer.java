...
import berlin.com.sun.tools.javac.util.JCDiagnostic;
...
public class Infer {
    ...
    /**
     * A mapping that returns its type argument with every UndetVar replaced by
     * its `inst' field. Throws a NoInstanceException if this not possible
     * because an `inst' field is null.
     */
    Mapping getInstFun = new Mapping("getInstFun") {
        public Type apply(Type t) {
            ...
            case UNDETVAR:
                ...
                if (that.inst == null)
                    throw ambiguousNoInstanceException.setMessage("type.variable.has.undetermined.type", that.qtype);
    ...
    /**
     * Instantiate undetermined type variable to its minimal upper bound. Throw
     * a NoInstanceException if this not possible.
     */
    void maximizeInst(UndetVar that, Warner warn) throws NoInstanceException {
        if (that.inst == null) {
            if (that.hibounds.isEmpty())
                ...
                for (List<Type> bs = that.hibounds; bs.nonEmpty() && that.inst == null; bs = bs.tail) {
                ...
                if (that.inst == null || !types.isSubtypeUnchecked(that.inst, that.hibounds, warn))
                    throw ambiguousNoInstanceException.setMessage("no.unique.maximal.instance.exists", that.qtype,
                            that.hibounds);
            }
        }
    }
    ...
    void minimizeInst(UndetVar that, Warner warn) throws NoInstanceException {
        if (that.inst == null) {
            if (that.lobounds.isEmpty())
                ...
                if (that.inst == null)
                    throw ambiguousNoInstanceException.setMessage("no.unique.minimal.instance.exists", that.qtype,
                            that.lobounds);
            }
            ...
            if (that.hibounds.tail.isEmpty())
                ...
                for (List<Type> bs = that.hibounds; bs.nonEmpty() && hb == null; bs = bs.tail) {
                    if (isSubClass(bs.head, that.hibounds))
                        hb = types.fromUnknownFun.apply(bs.head);
                }
            if (hb == null || !types.isSubtypeUnchecked(hb, that.hibounds, warn)
    ...
    /**
     * Try to instantiate expression type `that' to given type `to'. If a
     * maximal instantiation exists which makes this type a subtype of type
     * `to', return the instantiated type. If no instantiation exists, or if
     * several incomparable best instantiations exist throw a
     * NoInstanceException.
     */
    public Type instantiateExpr(ForAll that, Type to, Warner warn) throws NoInstanceException {
        ...
        if (!types.isSubtype(qtype1, to)) {
            throw unambiguousNoInstanceException
                    .setMessage("no.conforming.instance.exists", that.tvars, that.qtype, to);
        }
        for (List<Type> l = undetvars; l.nonEmpty(); l = l.tail)
    ...
    /**
     * Instantiate method type `mt' by finding instantiations of `tvars' so that
     * method can be applied to `argtypes'.
     */
    public Type instantiateMethod(List<Type> tvars, MethodType mt, List<Type> argtypes, boolean allowBoxing,
            boolean useVarargs, Warner warn) throws NoInstanceException {
    ...
    /**
     * Try to instantiate argument type `that' to given type `to'. If this
     * fails, try to insantiate `that' to `to' where every occurrence of a type
     * variable in `tvars' is replaced by an unknown type.
     */
    ...
    private void checkWithinBounds(List<Type> tvars, List<Type> arguments, Warner warn) throws NoInstanceException {
        for (List<Type> tvs = tvars, args = arguments; tvs.nonEmpty(); tvs = tvs.tail, args = args.tail) {
            ...
            if (!types.isSubtypeUnchecked(args.head, bounds, warn))
                throw unambiguousNoInstanceException.setMessage("inferred.do.not.conform.to.bounds", arguments, tvars);
        }
    }
}
