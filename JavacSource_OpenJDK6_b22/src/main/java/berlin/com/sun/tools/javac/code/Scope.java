...
import berlin.com.sun.tools.javac.util.*;

import java.util.Iterator;

/**
 * A scope represents an area of visibility in a Java program. The Scope class
 * is a container for symbols which provides efficient access to symbols given
 * their names. Scopes are implemented as hash tables. Scopes can be nested; the
 * next field of a scope points to its next outer scope. Nested scopes can share
 * their hash tables.
 */
public class Scope {
    ...
    /**
     * A linear list that also contains all entries in reverse order of
     * appearance (i.e later entries are pushed on top).
     */
    public Entry elems;
    ...
    /**
     * Construct a new scope, within scope next, with given owner, using given
     * table. The table's length must be an exponent of 2.
     */
    ...
    /**
     * Construct a fresh scope within this scope, with same owner, which shares
     * its table with the outer scope. Used in connection with method leave if
     * scope access is stack-like in order to avoid allocation of fresh tables.
     */
    public Scope dup() {
        Scope result = new Scope(this, this.owner, this.table);
        shared++;
        // System.out.println("====> duping scope " + this.hashCode() +
        // " owned by " + this.owner + " to " + result.hashCode());
        // new Error().printStackTrace(System.out);
        return result;
    }

    /**
     * Construct a fresh scope within this scope, with new owner, which shares
     * its table with the outer scope. Used in connection with method leave if
     * scope access is stack-like in order to avoid allocation of fresh tables.
     */
    public Scope dup(Symbol newOwner) {
        ...
        shared++;
        // System.out.println("====> duping scope " + this.hashCode() +
        // " owned by " + newOwner + " to " + result.hashCode());
        // new Error().printStackTrace(System.out);
        return result;
    }

    /**
     * Construct a fresh scope within this scope, with same owner, with a new
     * hash table, whose contents initially are those of the table of its outer
     * scope.
     */
    ...
    public Scope leave() {
        ...
        next.shared--;
        // System.out.println("====> leaving scope " + this.hashCode() +
        // " owned by " + this.owner + " to " + next.hashCode());
        // new Error().printStackTrace(System.out);
        return next;
    }
    ...
    /**
     * Enter symbol sym in this scope, but mark that it comes from given scope
     * `s' accessed through `origin'. The last two arguments are only used in
     * import scopes.
     */
    ...
    /**
     * Remove symbol from this scope. Used when an inner class attribute tells
     * us that the class isn't a package member.
     */
    ...
    /**
     * Given a class, is there already a class with same fully qualified name in
     * this (import) scope?
     */
    ...
    /**
     * Return the entry associated with given name, starting in this scope and
     * proceeding outwards. If no entry was found, return the sentinel, which is
     * characterized by having a null in both its scope and sym fields, whereas
     * both fields are non-null for regular entries.
     */
    ...
    public static class Entry {
        ...
        /**
         * The entry's scope. scope == null iff this == sentinel for an entry in
         * an import scope, this is the scope where the entry came from (i.e.
         * was imported from).
         */
        public Scope scope;
    ...
    public static class ImportScope extends Scope {
        ...
        public Entry lookup(Name name) {
            ...
            while (e.scope != null && (e.sym.name != name ||
            /*
             * Since an inner class will show up in package and import scopes
             * until its inner class attribute has been processed, we have to
             * weed it out here. This is done by comparing the owners of the
             * entry's scope and symbol fields. The scope field's owner points
             * to where the class originally was imported from. The symbol
             * field's owner points to where the class is situated now. This can
             * change when an inner class is read (see ClassReader.enterClass).
             * By comparing the two fields we make sure that we do not
             * accidentally import an inner class that started life as a flat
             * class in a package.
             */
        ...
        static class ImportEntry extends Entry {
            ...
            public Entry next() {
                ...
                while (e.scope != null && (e.sym.name != sym.name || e.sym.owner != e.scope.owner))
                    // see lookup()
                    e = e.shadowed;
                return e;
            }
    ...
    /**
     * An empty scope, into which you can't place anything. Used for the scope
     * for a variable initializer.
     */
