...
import berlin.com.sun.source.util.JavacTask;
import berlin.com.sun.source.util.TaskEvent;
import berlin.com.sun.source.util.TaskListener;
...
import berlin.com.sun.tools.javac.main.CommandLine;
import berlin.com.sun.tools.javac.main.JavaCompiler;
import berlin.com.sun.tools.javac.main.Main;
import berlin.com.sun.tools.javac.model.JavacElements;
import berlin.com.sun.tools.javac.model.JavacTypes;
import berlin.com.sun.tools.javac.parser.Parser;
import berlin.com.sun.tools.javac.parser.Scanner;
...
import berlin.com.sun.tools.javac.util.ClientCodeException;
...
public class JavacTaskImpl extends JavacTask {
    ...
    private berlin.com.sun.tools.javac.main.JavaCompiler compiler;
    ...
    JavacTaskImpl(JavacTool tool, Main compilerMain, Iterable<String> flags, Context context, Iterable<String> classes,
    ...
    private void prepareCompiler() throws IOException {
        if (!used.getAndSet(true)) {
            ...
            berlin.com.sun.tools.javac.parser.DocCommentScanner.Factory.preRegister(context);
            ...
            // TODO: should handle the case after each phase if errors have
            // occurred
            args = null;
        }
    }
     ...
     * @throws IOException
     *             TODO
     * @return a list of elements corresponding to the top level classes in the
    ...
    public Iterable<? extends TypeElement> enter(Iterable<? extends CompilationUnitTree> trees) throws IOException {
        ...
        if (trees == null) {
            ...
            if (notYetEntered.size() > 0) {
                if (!parsed)
                    parse(); // TODO would be nice to specify files needed to be
                             // parsed
                for (JavaFileObject file : fileObjects) {
     ...
     * previously been returned from {@link #enter}. If null is specified, all
     * outstanding classes will be analyzed.
     * 
     * @param classes
     *            a list of class elements
     */
    // This implementation requires that we open up privileges on JavaCompiler.
    // An alternative implementation would be to move this code to JavaCompiler
    // and
    // wrap it here
     ...
     * previously been returned from {@link #enter}. If there are classes
     * outstanding to be analyzed, that will be done before any classes are
     * generated. If null is specified, code will be generated for all
     * outstanding classes.
     * 
     * @param classes
     *            a list of class elements
    ...
    public Iterable<? extends JavaFileObject> generate(Iterable<? extends TypeElement> classes) throws IOException {
        ...
        try {
            analyze(null); // ensure all classes have been parsed, entered, and
                           // analyzed
