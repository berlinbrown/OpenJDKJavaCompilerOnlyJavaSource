...
/**
 * An extension to the base lexical analyzer that captures and processes the
 * contents of doc comments. It does so by translating Unicode escape sequences
 * and by stripping the leading whitespace and starts from each line of the
 * comment.
 * 
 * <p>
...
public class DocCommentScanner extends Scanner {
    ...
    /**
     * Create a scanner from the input buffer. buffer must implement array() and
     * compact(), and remaining() must be less than limit().
     */
    ...
    /**
     * Create a scanner from the input array. The array must have at least a
     * single character of extra space.
     */
    ...
    /**
     * The comment input buffer, index of next chacter to be read, index of one
     * past last character in buffer.
     */
    private char[] buf;
    ...
    /**
     * Process a doc comment and make the string content available. Strips
     * leading whitespace and stars.
     */
    ...
    protected void processComment(CommentStyle style) {
        ...
        while (bp < buflen) {
            ...
            if (ch == '*') {
                ...
                // The current line does not begin with a '*' so we will indent
                // it.
                for (int i = 1; i < col; i++) {
                    if (docCommentCount == docCommentBuffer.length)
                        expandCommentBuffer();
