...
public class Pretty extends JCTree.Visitor {
     ...
     * 
     * @param contextPrec
     *            The precedence level in force so far.
     * @param ownPrec
     ...
     * 
     * @param contextPrec
     *            The precedence level we revert to.
     * @param ownPrec
    ...
    /**
     * Derived visitor method: print list of statements, each on a separate
     * line.
     */
    ...
    public void printUnit(JCCompilationUnit tree, JCClassDecl cdef) throws IOException {
        ...
        for (List<JCTree> l = tree.defs; l.nonEmpty() && (cdef == null || l.head.getTag() == JCTree.IMPORT); l = l.tail) {
            if (l.head.getTag() == JCTree.IMPORT) {
                JCImport imp = (JCImport) l.head;
                Name name = TreeInfo.name(imp.qualid);
                if (name == name.table.asterisk || cdef == null || isUsed(TreeInfo.symbol(imp.qualid), cdef)) {
                    if (firstImport) {
                        firstImport = false;
                        println();
                    }
    ...
    public void visitMethodDef(JCMethodDecl tree) {
        try {
            ...
            if (tree.name == tree.name.table.init && enclClassName == null && sourceOutput)
                return;
            println();
            align();
            printDocComment(tree);
    ...
    public void visitNewClass(JCNewClass tree) {
        try {
            ...
            if (tree.def != null) {
                Name enclClassNamePrev = enclClassName;
                enclClassName = tree.def.name != null ? tree.def.name : tree.type != null
                        && tree.type.tsym.name != tree.type.tsym.name.table.empty ? tree.type.tsym.name : null;
